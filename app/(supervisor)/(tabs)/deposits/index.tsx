...
/**
 * Supervisor-side deposits history. Mirrors the approvals (retraits) screen
 * but uses the dépôts data flow. Tapping a row opens the shared
 * TransactionDetailSheet, since dépôts pre-completion are not actionable by
 * the supervisor (the workflow is OTP-only).
 */
export default function DepositsHistoryScreen() {
  ...
  const inFlight = byStatus.INITIATED.count + byStatus.CODE_VERIFIED.count;
  ...
  const filteredList = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter((d) =>
        `${d.client?.firstName ?? ''} ${d.client?.lastName ?? ''} ${d.collector?.firstName ?? ''} ${d.collector?.lastName ?? ''}`
          .toLowerCase()
          .includes(q),
      );
    }
      ...
      >
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.topBarMeta}>
            ...
            <Text style={styles.topBarTitle}>Historique dépôts</Text>
          </View>
          ...
          {totalCount === 0 ? (
            <View style={styles.donutEmpty}>
              ...
              <Text style={styles.emptyDesc}>
                Les statistiques apparaîtront avec le premier dépôt.
              </Text>
            </View>
          ) : (
        ...
        {isLoading ? (
          ...
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyTitle}>Aucun dépôt</Text>
            <Text style={styles.emptyDesc}>
              {search ? 'Essayez un autre mot-clé.' : 'Pas de dépôt correspondant à ce filtre.'}
            </Text>
          </View>
        ) : (
...
/**
 * Deposit-shaped → transaction-shaped adapter for the TransactionDetailSheet.
 * Once a deposit completes it has a transactionId; for in-flight deposits we
 * synthesize a Transaction-like object so the sheet can still render the
 * workflow / OTP / location panels.
 */
...
// ──────────────── Styles (cloned from approvals/index.tsx) ────────────────
...
const styles = StyleSheet.create({
  ...
  heroCard: {
    flexDirection: 'row',
    alignItems: 'stretch',
    backgroundColor: '#15803d', // green tint to distinguish from retraits hero
